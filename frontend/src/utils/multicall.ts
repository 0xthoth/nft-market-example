...
const multicall = async <T = any>(chainId: number, abi: any[], calls: Call[]): Promise<T> => {
  const multi = MULTICALL_CONTRACT.getEthersContract(chainId);
 ...
 */
export const multicallv2 = async <T = any>(chainId: number, abi: any[], calls: Call[], options?: MulticallOptions): Promise<T> => {
